...
```
<u>**CONCLUSIÓN**</u>:Para el método Promethee II, la mejor alternativa es A1 (Publicidad en Redes Sociales) ya que tiene el mayor flujo neto con un valor de 1.7500. Además, supera al resto de alternativas directamente. A2 no supera a ninguna otra alternativa, A3 supera a A2, a A4 y a A5, A4 supera a A2 y a A5 ,y A5 supera a A2. 
<br>
...
```
<br>
<u>**CONCLUSIÓN**</u>:Para el método Promethee II (medias), la mejor alternativa es A1 (Publicidad en Redes Sociales) ya que tiene el mayor flujo neto con un valor de 0.4375. Además, supera al resto de alternativas directamente. A2 no supera a ninguna otra alternativa, A3 supera a A2, a A4 y a A5, A4 supera a A2 y a A5 ,y A5 supera a A2. 
...
Xmat_criterios <- multicriterio.crea.matrizvaloraciones(
  c(1, 3/2, 2/3, 3, 5/4, 3/2,
    2/3, 1, 2/3, 2, 4/5, 1,
    3/2, 3/2, 1, 7/2, 7/5, 7/5,
    1/3, 1/2, 2/7, 1, 4/5, 1/2,
    4/5, 5/4, 5/7, 5/4, 1, 5/4,
    2/3, 1, 7/5, 1/2, 5/4, 1),
  6, n_criterios
)
...
# Criterio: Alcance
Xmat_alcance <- multicriterio.crea.matrizvaloraciones(
  c(1, 17/14, 17/12, 17/15, 17/18,
    14/17, 1, 14/12, 14/15, 14/18,
    12/17, 12/14, 1, 12/15, 12/18,
    15/17, 15/14, 15/12, 1, 15/18,
    18/17, 18/14, 18/12, 18/15, 1),
  5, n_alternativas
)
kable(Xmat_alcance, caption = "ALCANCE")

# Criterio: Costo
Xmat_costo <- multicriterio.crea.matrizvaloraciones(
  c(1, 2/3, 1/3, 5/6, 6/7,
    3/2, 1, 1/2, 5/8, 7/9,
    3, 2, 1, 5/2, 7/4,
    6/5, 8/5, 2/5, 1, 7/5,
...
# Criterio: ROI
Xmat_roi <- multicriterio.crea.matrizvaloraciones(
  c(1, 5/4, 3/2, 6/7, 10/13,
    4/5, 1, 6/5, 5/7, 13/10,
    2/3, 5/6, 1, 7/5, 7/10,
    7/6, 7/5, 5/7, 1, 13/7,
    13/10, 10/13, 10/7, 7/13, 1),
  5, n_alternativas
)
kable(Xmat_roi, caption = "ROI")

# Criterio: Duración
Xmat_duracion <- multicriterio.crea.matrizvaloraciones(
  c(1, 2/3, 5/7, 3/5, 1/2,
    3/2, 1, 7/5, 5/3, 2/5,
    7/5, 5/7, 1, 7/10, 5/9,
    5/3, 3/5, 10/7, 1, 3/5,
...
Xmat_segmentacion <- multicriterio.crea.matrizvaloraciones(
  c(1, 9/8, 8/7, 9/10, 10/7,
    8/9, 1, 7/6, 9/11, 11/8,
    7/8, 6/7, 1, 8/9, 9/7,
    10/9, 11/9, 9/8, 1, 7/6,
    7/10, 8/11, 7/9, 6/7, 1),
...
Xmat_flexibilidad <- multicriterio.crea.matrizvaloraciones(
  c(1, 4/3, 3/2, 5/4, 6/5,
    3/4, 1, 2/3, 4/5, 5/6,
    2/3, 3/2, 1, 3/4, 4/5,
    4/5, 5/4, 4/3, 1, 3/2,
...
```
<br>La mejor alternativa es A4 (Publicidad en Buscadores) con el mayor valor y la peor A2 (Marketing de contenidos) con el menor.
...
pesos.criterios
```
El criterio con mayor importancia es el ROI, con un peso de 0.2454, destacándose por encima de los demás criterios. En cambio, Duración es la menos relevante, con un peso de 0.0815, ya que tiene un impacto mínimo en la decisión final. Costo (0.1457), Alcance (0.2082), Flexibilidad (0.1609) y Segmentación (0.1583) tienen una importancia intermedia, con valores similares entre ellos, aunque el ROI y el Alcance sobresalen ligeramente.
...
pesos.alcance
```
Si solo consideramos el Alcance como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A5(0.2368) > A1(0.2237) > A4(0.1974) > A2(0.1842) > A3(0.1579).
...
pesos.costo
```
Si solo consideramos el Costo como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A3(0.3573) > A4(0.1915) > A5(0.1694) > A2(0.1532) > A1(0.1285).
...
pesos.ROI
```
Si solo consideramos el ROI como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A4(0.2354) > A1(0.2048) > A5(0.1903) > A2(0.1894) > A3(0.1799).
...
pesos.duracion
```
Si solo consideramos la Duración como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A5(0.3254) > A2(0.2056) > A4(0.1857) > A3(0.1558) > A1(0.1273).
...
pesos.segmentacion
```
Si solo consideramos la Segmentación como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A4(0.2234) > A1(0.2191) > A2(0.2047) > A3(0.1923) > A5(0.1603).
...
pesos.flexibilidad
```
Si solo consideramos la Flexibilidad como criterio para la elección de la mejor alternativa, los resultados obtenidos para la elección de la mejor alternativa son: A1(0.2449) > A4(0.2266) > A5(0.1901) > A3(0.1790) > A2(0.1591).
...
```
La alternativa con mayor ponderador global es A4 (0.2137), es decir, Publicidad en Buscadores. En consecuencia, podemos considerar esta alternativa como la más preferida en función de los criterios que hemos considerado. 
En segundo lugar se encuentra la alternativa 5 (0.2031), en tercer lugar la alternativa 3 (0.2010) y en cuarto lugar la alternativa 4 (0.2).
En último lugar se encuentra A2 (0.1819), es decir, Marketing de Contenidos. En consecuencia, podemos decir que esta alternativa es la menos preferida en función de los criterios que hemos considerado.
...
### 3.2 Librería AHP
```{r}
# install.packages("remotes") # Instalar remotes si no lo tienes
# remotes::install_github("gluc/ahp", force = TRUE) # Instalar ahp desde GitHub
library(ahp) 
